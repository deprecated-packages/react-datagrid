...
var Select = function (_React$Component) {
	...
	_createClass(Select, [{
		...
		value: function componentDidUpdate() {
			...
			if (this.input) {
				var number = this.component.clientWidth - this.caret.clientWidth;
				if (this.tags) {
		...
		value: function onClickTag(value) {
			...
			var values = this._getSanitizeValue().filter(function (v) {
				return v[_this3.props.valueKey] !== value;
			});
			this._saveValue(values);
		}
	}, {
		...
		value: function renderTag(i, key) {
			...
			return _react2.default.createElement(
				"span",
				{ className: "tag", key: key, ref: function ref(tags) {
						return _this4.tags = tags;
					} },
		...
		value: function _loadData(input) {
			...
			this.props.loadOptions(input, function (err, data) {
				if (!err) {
					_this5._setupSimpleValueLabelCache(data.options);
					var options = data.options.filter(_this5.selectValuesFilter.bind(_this5, _this5._getSanitizeValue()));
					if (_this5.props.creatable && options.length === 0) {
						var _options$push;

						options.push((_options$push = {
							creatable: true
						}, _defineProperty(_options$push, _this5.props.valueKey, "create"), _defineProperty(_options$push, _this5.props.labelKey, _this5.props.promptTextCreator(input)), _options$push));
					}
		...
		value: function _setupSimpleValueLabelCache(options) {
			...
			if (this.props.simpleValue) {
				...
				options.forEach(function (o) {
					simpleValueCache[o[_this6.props.valueKey]] = o[_this6.props.labelKey];
				});
			...
			if (this.props.simpleValue) {
				if (this.props.multi) {
					this.props.onChange(value.map(function (v) {
						return v[_this7.props.valueKey];
					}));
				} else {
			...
			if (this.props.multi) {
				if (v.hasOwnProperty(this.props.valueKey)) {
					return !itemSelected.map(function (x) {
						return x[_this8.props.valueKey] || !x.hasOwnProperty(_this8.props.valueKey);
		...
		value: function render() {
			...
			return _react2.default.createElement(
				...
				_react2.default.createElement(
					"div",
					{ className: "select-line", style: { flex: "" }, onClick: this.onClickSelectLine },
					this.renderTags(),
					this.renderInput(),
					_react2.default.createElement(
						"div",
						{ className: "caret-btn", ref: function ref(caret) {
								return _this10.caret = caret;
							}, onClick: this.onCaretDown },
